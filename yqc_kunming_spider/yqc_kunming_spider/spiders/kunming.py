...
from yqc_kunming_spider.items import YqcKunmingSpiderItem
...
class KunmingSpider(CrawlSpider):
    name = 'kunming'
    allowed_domains = ['km.gov.cn']
    start_urls = [
        'http://www.km.gov.cn/zfxxgk/fdzdgknr/zfwj/']

    rules = (
        Rule(LinkExtractor(allow=r'http://www.km.gov.cn/c.*shtml'),
             callback='parse_page',
             follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        title = response.xpath("//*[@class='content']/h1/text()").get()
        cont = response.xpath("//*[@class='content']").get()
        index_id = str('_NULL')
        pub_org = response.xpath("/html/body/div[1]/div[2]/div[2]/div[2]/div[1]/table/tbody/tr[2]/td[2]").get()

        pub_time = response.xpath("//*[@class='content']/div[@class='activity-info']/span[1]/text()").get()
        doc_id = response.xpath("/html/body/div[1]/div[2]/div[2]/div[2]/div[1]/table/tbody/tr[4]/td[2]/text()").get()
        region = str('昆明')
        ...
        item = YqcKunmingSpiderItem(cont_dict=self.cont_dict)
    ...
    def parse_page(self, response):
        ...
        if str('currentPage') in url:
            ...
            doc_id = response.xpath(
                "/html/body/div[1]/div[2]/div[2]/div[2]/div[1]/table/tbody/tr[4]/td[2]/text()").get()
            region = str('昆明')
            update_time = datetime.datetime.now().strftime("%Y-%m-%d 00:00:00")
