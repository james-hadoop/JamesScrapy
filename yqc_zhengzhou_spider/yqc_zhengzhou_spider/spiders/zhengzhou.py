...
from yqc_zhengzhou_spider.items import YqcZhengzhouSpiderItem
...
class ZhengzhouSpider(CrawlSpider):
    name = 'zhengzhou'
    allowed_domains = ['zhengzhou.gov.cn']
    start_urls = ['http://public.zhengzhou.gov.cn/?a=theme&n=3']

    rules = (
        Rule(LinkExtractor(allow=r'.*public.zhengzhou.gov.cn/0.*'),
             callback='parse_page',
             follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        item = YqcZhengzhouSpiderItem(cont_dict=self.cont_dict)
    ...
    def parse_page(self, response):
        ...
        if str('currentPage') in url:
            ...
            title = response.xpath("/html/body/div[2]/div[3]/div/div[2]/div[1]/text()").get()
            cont = str('_NULL')
            index_id = response.xpath("/html/body/div[2]/div[3]/div/div[1]/ul/li[1]/em/text()").get()
            pub_org = response.xpath("/html/body/div[2]/div[3]/div/div[1]/ul/li[3]/em/text()").get()

            pub_time = response.xpath("/html/body/div[2]/div[3]/div/div[1]/ul/li[7]/em/text()").get()
            doc_id = str('_NULL')
            region = str('郑州')
            update_time = datetime.datetime.now().strftime("%Y-%m-%d 00:00:00")
            ...
            for key in keys:
                if key in title:
                    # print("\t>>> included")
                    self.dict_add_one(re.sub('[\s+]', ' ', title), response.url, cont,
