...
from yqc_nanchang_spider.items import YqcNanchangSpiderItem
...
class NanchangSpider(CrawlSpider):
    name = 'nanchang'
    allowed_domains = ['nc.gov.cn']
    start_urls = ['http://www.nc.gov.cn/ncszf/szfwj/nav_list.shtml']

    rules = (
        Rule(LinkExtractor(allow=r'.*nc.gov.cn/xxgkpt/qtygwj/2020.*'),
             callback='parse_page',
             follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        item = YqcNanchangSpiderItem(cont_dict=self.cont_dict)
    ...
    def parse_page(self, response):
        ...
        if str('currentPage') in url:
            ...
            title = response.xpath(
                "/html/body/div[2]/div[2]/div/div[1]/ucapcontent/table/tbody/tr[3]/td[2]/text()").get()
            cont = response.xpath("//*[@id='zoomcon']").get()
            index_id = response.xpath(
                "/html/body/div[2]/div[2]/div/div[1]/ucapcontent/table/tbody/tr[1]/td[2]/text()").get()
            pub_org = response.xpath(
                "/html/body/div[2]/div[2]/div/div[1]/ucapcontent/table/tbody/tr[2]/td[2]/text()").get()

            pub_time = response.xpath(
                "/html/body/div[2]/div[2]/div/div[1]/ucapcontent/table/tbody/tr[2]/td[4]/text()").get()
            doc_id = response.xpath(
                "/html/body/div[2]/div[2]/div/div[1]/ucapcontent/table/tbody/tr[4]/td[2]/text()").get()
            region = str('南昌')
            update_time = datetime.datetime.now().strftime("%Y-%m-%d 00:00:00")
