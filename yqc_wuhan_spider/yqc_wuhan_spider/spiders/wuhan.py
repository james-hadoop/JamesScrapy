...
from yqc_wuhan_spider.items import YqcWuhanSpiderItem
...
class WuhanSpider(CrawlSpider):
    ...
    start_urls = ['http://www.wuhan.gov.cn/zwgk/?flag=6376']

    rules = (
        Rule(LinkExtractor(allow=r'.*wuhan.gov.cn/zwgk/xxgk/zfwj/zfgz/2020.*'),
             callback='parse_page',
             follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        item = YqcWuhanSpiderItem(cont_dict=self.cont_dict)
    ...
    def parse_page(self, response):
        ...
        if str('currentPage') in url:
            ...
            title = response.xpath("/html/body/div[4]/div[2]/div/h2/text()").get()
            cont = response.xpath("/html/body/div[4]/div[2]/div/div[2]/div/div[1]/div/div[2]").get()
            index_id = response.xpath("/html/body/div[4]/div[2]/div/div[1]/ul/li[1]/text()").get()
            pub_org = response.xpath("/html/body/div[4]/div[2]/div/div[1]/ul/li[2]/text()").get()

            pub_time = response.xpath("/html/body/div[4]/div[2]/div/div[1]/ul/li[6]/text()").get()
            doc_id = response.xpath("/html/body/div[4]/div[2]/div/div[1]/ul/li[3]/text()").get()
            region = str('武汉')
            update_time = datetime.datetime.now().strftime("%Y-%m-%d 00:00:00")
