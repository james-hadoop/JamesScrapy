...
from yqc_chengdu_spider.items import YqcChengduSpiderItem
...
class ChengduSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=r'.*chengdu.gov.cn/chengdu/c131029.*'),
             callback='parse_page',
             follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        index_id = str("_NULL")
        pub_org = response.xpath("//*[@id='zc']/div[3]/table/tbody/tr[1]/td[2]/text()").get()

        pub_time = response.xpath("//*[@id='zc']/div[3]/table/tbody/tr[2]/td[1]/text()").get()
        doc_id = response.xpath("//*[@id='zc']/div[3]/table/tbody/tr[1]/td[1]/text()").get()
        region = str('成都')
