...
from yqc_shenyang_spider.items import YqcShenyangSpiderItem
...
class ShenyangSpider(CrawlSpider):
    name = 'shenyang'
    allowed_domains = ['shenyang.gov.cn']
    start_urls = [
        'http://www.shenyang.gov.cn/syszf/zwgk/glist.html']

    rules = (
        Rule(LinkExtractor(allow=r'.*shenyang.gov.cn/html/SY.*'),
             callback='parse_page',
             follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        title = response.xpath("//*[@id='content']/div/div/div[3]/em/text()").get()
        cont = response.xpath("//*[@id='content']/div/div/div[6]").get()
        index_id = str('_NULL')
        pub_org = response.xpath("//*[@id='content']/div/div/div[5]/em/em/text()").get()

        pub_time = response.xpath("//*[@id='content']/div/div/div[5]/em/em/text()").get()
        doc_id = response.xpath("//*[@id='content']/div/div/div[4]/span/em/text()").get()
        region = str('沈阳')
        ...
        item = YqcShenyangSpiderItem(cont_dict=self.cont_dict)
