...
class YqcXiamenSpiderPipeline(object):
    def __init__(self):
    ...
    def sql(self):
        if not self._sql:
            self._sql = """
               insert into yqc_spider_xiamen(id, title, url, pub_time, pub_org, doc_id, index_id, key_cnt, region, update_time, cont) values (null, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
               """
            return self._sql
        return self._sql
