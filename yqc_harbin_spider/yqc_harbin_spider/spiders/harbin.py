...
from yqc_harbin_spider.items import YqcHarbinSpiderItem
...
class HarbinSpider(CrawlSpider):
    ...
    start_urls = ['http://www.harbin.gov.cn/col/col15/index.html']

    rules = (
        Rule(LinkExtractor(allow=r'.*harbin.gov.cn/art/2020.*'),
             callback='parse_page',
             follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        index_id = str('_NULL')
        pub_org = response.xpath("//*[@id='zoom']/p[15]/text()[1]").get()

        pub_time = response.xpath("//*[@id='zoom']/p[15]/text()[2]").get()
        doc_id = str('_NULL')
        region = str('哈尔滨')
        ...
        item = YqcHarbinSpiderItem(cont_dict=self.cont_dict)
    ...
    def parse_page(self, response):
        ...
        if str('currentPage') in url:
            ...
            title = response.xpath("//*[@id='zoom']/p[1]/span/strong/text()").get()
            cont = response.xpath("//*[@id='zoom']").get()
            index_id = response.xpath("/html/body/div[5]/div[1]/div[4]/table[1]/tbody/tr[2]/td[3]/text()").get()
            pub_org = str('_NULL')

            pub_time = response.xpath("/html/body/div[5]/div[1]/div[4]/table[1]/tbody/tr[3]/td[3]/text()").get()
            doc_id = response.xpath("/html/body/div[5]/div[1]/div[4]/table[1]/tbody/tr[4]/td[3]/text()").get()
            region = str('哈尔滨')
            update_time = datetime.datetime.now().strftime("%Y-%m-%d 00:00:00")
